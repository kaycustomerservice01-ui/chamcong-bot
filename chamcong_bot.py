...
# States
(CHON_LOAI, CHON_CA, CHON_NGHI,
 CHON_NGAY_BU, CHON_CA_BU, NHAP_LY_DO_BU) = range(6)
...
async def chon_loai(update, ctx):
    ...
    if "Nghỉ" in text:
        ...
        return CHON_NGHI
    if "Chấm Bù" in text or "Cham Bu" in text:
        await update.message.reply_text(
...
async def chon_nghi(update, ctx):
    ...
    user  = update.effective_user
    if "Huỷ" in ly_do or "Huy" in ly_do:
        await update.message.reply_text("❌ Đã huỷ.", reply_markup=RKR)
        return ConversationHandler.END
    dt      = now_vn()
...
def main():
    ...
    conv = ConversationHandler(
        ...
        fallbacks=[
            ...
            CommandHandler("chamcong", chamcong_start),
        ],
        allow_reentry=True,
    )
